<div class="space-y-2">
    ...
    <div wire:key="string-item-{{ $index }}">
        <x-input wire:model.live.debounce.300ms="items.{{ $index }}.value" placeholder="{{ $placeholder }}">
            @if(count($items) > 1)
            <x-slot:append>
                {{-- Add `join-item` to make it visually connected --}}
                <button type="button" wire:click="removeRow({{ $index }})" class="join-item btn btn-error">
