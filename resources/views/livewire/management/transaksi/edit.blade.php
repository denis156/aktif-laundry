<div>
    <x-header title="Edit Transaksi" separator progress-indicator>
        ...
        <x-slot:actions>
            <livewire:management.component.whats-app-button :transaksi-id="$transaksiId" size="md"
                :key="'wa-edit-'.$transaksiId" />
    ...
    <x-form wire:submit="save" no-separator>
        ...
        <div class="lg:grid grid-cols-5">
            ...
            <x-card class="col-span-3">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-input label="Kode Transaksi" wire:model="formData.kode_transaksi" readonly
                        hint="Kode tidak dapat diubah" icon="o-hashtag" />

                    <x-input label="Kasir" value="{{ Auth::user()->name ?? 'Tidak diketahui' }}" readonly
                        hint="Tidak dapat diubah" icon="o-user-circle" />
                </div>
        ...
        <div class="lg:grid grid-cols-5 mt-8">
            ...
            <x-card class="col-span-3">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <x-input label="Diskon" type="number" wire:model.live="formData.diskon" placeholder="0" prefix="Rp"
                        hint="Opsional" />
                ...
                <x-textarea label="Catatan" wire:model="formData.catatan" placeholder="Catatan tambahan..." rows="3"
                    hint="Opsional" />
            </x-card>
        </div>
        ...
        <div class="lg:grid grid-cols-5 mt-8">
            ...
            <x-card class="col-span-3">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-select label="Kode Promo" wire:model.live="selectedPromoId" icon="o-ticket"
                        :options="$promoOptions" option-value="id" option-label="name" placeholder="Pilih promo..."
                        hint="Diskon otomatis diterapkan" />

                    <x-select label="Kode Referral" wire:model="selectedReferralId" icon="o-user-group"
