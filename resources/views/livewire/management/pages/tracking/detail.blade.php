<div>
    {{-- HEADER --}}
    <x-header title="Kurir: {{ $kurir?->nama }}" subtitle="Pantau lokasi kurir secara real-time" separator progress-indicator>
        <x-slot:actions>
            <x-button label="Kembali" icon="o-arrow-left" link="{{ route('tracking.index') }}" class="btn-secondary"
                responsive />
        </x-slot:actions>
    </x-header>
    ...
    <x-card class="shadow-sm" body-class="relative w-full h-[78dvh] rounded-lg overflow-hidden">
        {{-- Map Element --}}
        <div id="map-tracking-detail" wire:ignore class="w-full h-full z-0"></div>

        {{-- Zoom Controls - KIRI ATAS --}}
        <div class="absolute top-4 left-4 z-1">
            <livewire:components.maps-controller :map-id="'map-tracking-detail'" position="top-left" :show-zoom="true"
                :show-layers="false" :show-center="false" :show-compass="false" />
        </div>

        {{-- Speed Indicator - TENGAH ATAS --}}
        <div class="absolute top-4 left-1/2 -translate-x-1/2 z-1" wire:poll.visible="updateTrackingData">
        ...
        <div class="absolute top-4 right-4 z-1">
            <livewire:components.maps-controller :map-id="'map-tracking-detail'" position="top-right" :show-zoom="false"
                :show-layers="true" :show-center="true" :show-compass="true" :current-layer="'googleTraffic'" />
        </div>
    </x-card>
    @else
    <div class="flex items-center justify-center h-[78dvh] bg-base-200 rounded-lg">
        <div class="text-center space-y-3 p-4">
            ...
            <div class="space-y-1">
                ...
                <p class="text-xs text-base-content/40">{{ $kurir?->nama }} belum mulai tracking</p>
            </div>
        </div>
    </div>
    @endif
...
<script>
    let mapManager = null;
            ...
            function initTrackingDetailMap() {
                const mapElement = document.getElementById('map-tracking-detail');
                if (!mapElement || mapManager) return;

                // Wait for Maps to be available
                if (typeof window.Maps === 'undefined') {
                    setTimeout(initTrackingDetailMap, 100);
                    return;
                }
                ...
                mapManager = window.Maps.createMap('map-tracking-detail', {
                    latitude: pelangganLat,
                    longitude: pelangganLng,
                    zoom: zoom.city,
                    defaultTileLayer: 'googleTraffic',
                    rotate: false, // No rotation for management view
                    enableCompass: true,
                    smoothCompass: true,
                ...
                console.log('Tracking detail map initialized');
            }
