<div wire:poll.visible.30s>
    <!-- HEADER -->
    <x-header title="Layanan" icon="o-sparkles" icon-classes="bg-primary text-primary-content rounded-full p-1 w-8 h-8" subtitle="Jenis Cuci & Tarif Harga" separator progress-indicator>
        <x-slot:middle class="justify-end">
            <x-input placeholder="Cari layanan..." wire:model.live.debounce="search" clearable icon="o-magnifying-glass" />
        </x-slot:middle>
        <x-slot:actions>
            <x-button label="Tambah Layanan" link="{{ route('layanan.create') }}" wire:navigate.hover responsive icon="o-plus" class="btn-success" />
        ...
        <x-table :headers="$headers" :rows="$layanan" :sort-by="$sortBy" striped with-pagination per-page="perPage" :per-page-values="[5, 10, 25, 50]">
            ...
            <div class="flex items-center justify-end gap-2">
                <x-button
                    label="Edit"
                    icon="o-pencil"
                    link="{{ route('layanan.edit', $item->id) }}"
                    wire:navigate.hover
    ...
    <x-modal wire:model="deleteModal" box-class="max-w-md" class="modal-bottom sm:modal-middle backdrop-blur-md">
        <div class="text-center space-y-2">
            ...
            <div>
                ...
                <p class="text-sm text-base-content mt-2">Apakah Anda yakin ingin menghapus layanan <span class="font-bold">{{ $deleteName }}</span> ?</p>
            </div>
        </div>
