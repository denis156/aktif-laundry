<div wire:poll.visible.30s>
    <!-- HEADER -->
    <x-header title="Transaksi" icon="o-clipboard-document-list" icon-classes="bg-primary text-primary-content rounded-full p-1 w-8 h-8" subtitle="Riwayat & Manajemen Pesanan" separator progress-indicator>
    ...
    <x-card class="shadow-sm">
        <x-table :headers="$headers" :rows="$transaksi" :sort-by="$sortBy" striped with-pagination per-page="perPage" :per-page-values="[5, 10, 25, 50]" link="{{ route('transaksi.edit', '[id]') }}">
            <x-slot:empty>
