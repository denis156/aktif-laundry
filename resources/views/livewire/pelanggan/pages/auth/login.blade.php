<div>
    <x-card class="bg-base-200" body-class="space-y-6" title="Selamat Datang Kembali"
        ...
        <x-form wire:submit="login">
            <!-- Email Input -->
            <x-input label="Email" type="email" wire:model="email" placeholder="nama@email.com" icon="o-envelope"
                required autofocus class="input-lg" />

            <!-- Password Input -->
            <x-password label="Password" wire:model="password" placeholder="••••••••" icon="o-lock-closed" right
            ...
            <x-slot:actions>
                <div class="space-y-3 w-full pt-2">
                    ...
                    <a href="{{ route('auth.google') }}"
                        class="btn bg-white text-black border-[#e5e5e5] btn-lg btn-block">
                    ...
                    <div class="text-center text-sm">
                        ...
                        <a href="{{ route('register.pelanggan') }}" class="link link-primary font-semibold"
                            wire:navigate>
                            Daftar di sini
                        </a>
                    </div>
    ...
    {{-- Modal In-App Browser iOS Instructions --}}
    <x-modal wire:model="showInAppBrowserModal" title="Buka di Browser Eksternal"
        class="modal-bottom w-full backdrop-blur" persistent>
        <div class="space-y-4" x-data="{
            browserName: '',
            init() {
                const detection = window.browserHelper.detect();
                this.browserName = window.browserHelper.getBrowserName(detection.browser);
            }
        }">
            ...
            <div class="text-center space-y-2">
                <p class="text-sm text-base-content/70">
                    Anda membuka dari <span class="font-semibold" x-text="browserName"></span>.
                </p>
                <p class="text-sm text-base-content/70 font-semibold">
                    Login Google hanya bekerja di browser eksternal (Safari/Chrome).
                </p>
            </div>
            ...
            <div class="bg-base-200 rounded-lg p-4 space-y-2">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <p class="font-bold text-sm">Cara Membuka di Safari/Chrome:</p>
                </div>
                <ol class="list-decimal list-inside space-y-1 text-sm text-base-content/70 ml-2">
                    ...
                    <li>Pilih <span class="font-semibold">"Open in Safari"</span> atau <span class="font-semibold">"Open
                            in Chrome"</span></li>
                    <li>Setelah terbuka di browser eksternal, login dengan Google akan berfungsi</li>
                </ol>
            </div>

            <div class="alert alert-info">
                <x-icon name="iconpark.info-o" class="w-5 h-5" />
                <span class="text-xs">Login manual dengan email & password tetap bisa digunakan di sini</span>
            </div>
        </div>

        <x-slot:actions>
            <x-button label="Mengerti" class="btn-primary btn-block" @click="$wire.showInAppBrowserModal = false" />
        </x-slot:actions>
    </x-modal>
</div>
