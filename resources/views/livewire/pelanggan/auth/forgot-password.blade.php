<x-card class="bg-base-200" body-class="space-y-6" title="Lupa Password"
    ...
    <x-form wire:submit="sendResetLink">
        ...
        <x-slot:actions>
            <div class="space-y-3 w-full pt-2">
                ...
                <x-button label="Kembali ke Login" link="{{ route('login.pelanggan') }}"
                    class="btn-neutral btn-soft btn-lg btn-block" icon="o-arrow-left" wire:navigate />
            </div>
        </x-slot:actions>
    </x-form>
    ...
    <div class="text-center text-xs">
        <p>{{ config('app.name') }} Developed by <span class="font-bold text-primary uppercase">Team {{ config('app.name') }}</span>
        </p>
    </div>
</x-card>
