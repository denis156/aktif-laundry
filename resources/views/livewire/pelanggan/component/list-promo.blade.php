                ...
                <x-card title="{{ $promo['nama_promo'] }}" subtitle="{{ $promo['kode_promo'] }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-base font-semibold text-info">{{ $promo['nilai_diskon_formatted'] }}</span>
                        </div>
                        ...
                        @if ($promo['min_transaksi'])
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs text-base-content/60">Min. {{ $promo['min_transaksi_formatted']
                                    }}</span>
                            </div>
                        @endif
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs text-base-content/60">{{ $promo['berlaku_untuk_label'] }}</span>
                        </div>

                        {{-- Tanggal Berakhir --}}
                        <div class="flex items-center gap-2">
                            <x-icon name="iconpark.time-o" class="w-4 h-4 text-warning" />
                            <span class="text-xs text-base-content/60">Berlaku s/d {{ $promo['tanggal_berakhir']
                                }}</span>
                        </div>
                        ...
                        @if ($promo['kuota_total'])
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs text-base-content/60">Tersisa {{ $promo['kuota_tersisa'] }}
                                    kuota</span>
                            </div>
                        @endif
                    ...
                    <x-slot:figure>
                        <div class="border-b-2 border-dashed border-info aspect-3/2 w-full bg-base-200">
                            <img src="{{ $promo['banner_url'] }}" alt="{{ $promo['nama_promo'] }}"
